...
- [2026-09-11] "Ruined all my presets" had TWO bugs stacked: a real one (B114, the anchor latch) that my first probe found because it was the one my scenario could show, and the actual root cause (B115) that only the human's OWN preset files revealed — the fresh-defaults rig can never hold a corner array saved under an older layout. Rule: when the report says "my presets", load the presets (`~/Library/Application Support/…`) before theorising; a probe on defaults answers a different question. And: ADR-104's "append-only" lived in a comment for 20 days without an oracle — the order was derived from a table walk, so the promise was one row-insert from false. A promise about a derived order needs a frozen fixture, not prose. Library candidate; falsifier: a comment-only invariant that survives a table change.
